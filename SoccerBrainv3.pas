//{$define TACKLE_FAILED}   // sempre fallo
...
const xp_TAL_GOALKEEPER_POINTS = 120;
const xp_TAL_CHALLENGE_POINTS = 120;
const xp_TAL_TOUGHNESS_POINTS = 120;
const xp_TAL_POWER_POINTS = 120;
const xp_TAL_CROSSING_POINTS = 120;
const xp_TAL_LONGPASS_POINTS = 120;
const xp_TAL_EXPERIENCE_POINTS = 120;
const xp_TAL_DRIBBLING_POINTS = 120;
const xp_TAL_BULLDOG_POINTS = 120;
const xp_TAL_MIDOFFENSIVE_POINTS = 120;
const xp_TAL_MIDDEFENSIVE_POINTS = 120;
const xp_TAL_BOMB_POINTS = 120;
const xp_TAL_PLAYMAKER_POINTS = 120;
const xp_TAL_FAUL_POINTS = 120;
const xp_TAL_MARKING_POINTS = 120;
const xp_TAL_Positioning_POINTS = 120;
const xp_TAL_FREEKICKS_POINTS = 120;
...
TSoccerPlayer = class
  ...
  public
    ...
    Tal_longpass: integer;  // +1 distanza passaggi
    Tal_Toughness: integer;  // +1 tackle
    Tal_Challenge: integer;  // lottatore + 1 autotackle
    Tal_Power : integer;   // toughness +1 resist takcle
    Tal_Crossing : integer;  // +1 crossing
    Tal_Experience: integer; // pressing non costa mosse
    Tal_Dribbling: integer;  // +1 dribbling
    Tal_Bulldog: Integer; // mastino +1 intercept
    Tal_GoalKeeper: integer;  // può giocare in porta
    Tal_midOffensive: Integer; // durante ai_moveall tende ad attaccare
    Tal_midDefensive: Integer; // durante ai_moveall tende a difendere
    Tal_Bomb: Integer;        // tal_bomb è un +1 quando si buffa con corsa o riceve shp o vince tackle o vince dribbling
    Tal_PlayMaker: integer;
    Tal_faul: Integer;
    Tal_marking: Integer;
    Tal_Positioning: Integer;
    Tal_freekicks: Integer;
    ...
    xpTal_Bomb: Integer;
    xpTal_PlayMaker: integer;
    xpTal_faul: Integer;
    xpTal_marking: Integer;
    xpTal_Positioning: Integer;
    xpTal_freekicks: Integer;
    ...
    constructor create ( const aTeam, aGuidTeam, aMatchesPlayed : integer; const aIds, aName, aSurname, aTalents, AT: string );
    destructor Destroy; override;
    ...
    property CellX : ShortInt read cx write SetCellX;
    property CellY : ShortInt read cy write SetCellY;
    property Talents : shortstring read fTalents write LoadTalents;
    ...
    property Stamina : ShortInt read fStamina write SetStamina;
  ...
  protected
    public
      ...
      dir_log: string;
      ReserveSlot : array [0..1, -4..-1, 0..6] of string;
      ReserveSlotTV : array [0..1, -4..-1, 0..6] of TPoint;
      AIField: array [0..1,0..6,0..11] of TPoint;
      ...
      OnNotifyFileData: TNotifyFileData;
      //FormationCells: TList<TFormationCell>;
...
//      InterceptMalus
      ToEmptyCellMalus: integer;                          // definizioni
      ...
      tsScript: TstringList;   // la lista di ciò che accade sul server viene spedita al client
...
//      function FieldCellToFormationCell ( CellX, CellY : integer ): TFormationCell;
    ...
    function GetSoccerPlayerInGame (ids: string): TSoccerPlayer;
...
begin
  subCell := SE_recordList.CreateList ( sizeof (Tpoint));
end;
destructor TShotCell.Destroy;
begin
  //while SubCell.Count > 0 do begin
  //  subcell.Delete(0)
  //end;
  //subCell.Free ;
  inherited;
...
procedure TSoccerPlayer.LoadTalents (  v: shortstring );
begin
  fTalents := v;

  if fTalents = 'goalkeeper' then  Tal_GoalKeeper:=1;
  if fTalents = 'challenge' then  tal_Challenge:=1;
  if fTalents = 'toughness' then  tal_Toughness:=1;
  if fTalents = 'power' then  tal_Power:=1;
  if fTalents = 'crossing' then  tal_Crossing:=1;
  if fTalents = 'longpass' then  tal_longpass:=1;
  if fTalents = 'experience' then  Tal_Experience:=1;
  if fTalents = 'dribbling' then  Tal_Dribbling:=1;
  if fTalents = 'bulldog' then  Tal_Bulldog:=1;
  if fTalents = 'offensive' then  Tal_midOffensive:=1;
  if fTalents = 'defensive' then  Tal_midDefensive:=1;
  if fTalents = 'bomb' then  Tal_Bomb:=1;
  if fTalents = 'playmaker' then  Tal_PlayMaker:=1;
  if fTalents = 'faul' then  Tal_faul:=1;
  if fTalents = 'marking' then  Tal_marking:=1;
  if fTalents = 'Positioning' then  Tal_Positioning:=1;
  if fTalents = 'freekicks' then  Tal_freekicks:=1;

  if (Tal_GoalKeeper <= 0) and (Role = 'G') then begin
    DefaultDefense:= DefaultDefense - 3;
    if DefaultDefense <= 0 then DefaultDefense:=1;
    Defense:= DefaultDefense;
  end;
  //  Tal_PlayMaker:= StrToBool(ts.Values ['PlayMaker']);

//  Flank:= StrToIntDef(ts.Values ['Flank'],0);
//  Positioning:= StrToIntDef(ts.Values ['Positioning'],0);

//  LowThroughBall:= StrToIntDef(ts.Values ['Low.Through.Ball'],0);
//  Bomb:= StrToIntDef(ts.Values ['Bomb'],0);

//  Diving:=StrToIntDef(ts.Values ['Diving'],0);// risponde a PrecisionShot,PowerShot,
//  Reflexes:=StrToIntDef(ts.Values ['Reflexes'],0);// risponde a Heading
//  Positioning:= StrToIntDef(ts.Values ['Positioning'],0);// risponde a Bomb
...
procedure TSoccerPlayer.SetStamina( v: ShortInt );
begin
  fStamina := v;
...
begin
  ...
  PlayerA.AIFormationCellY  := aPoint.X;;
...
begin
   ...
   (* COF, FKF2 *)
    ...
    function (const L, R: TSoccerPlayer): integer
    begin
      Result := (R.Passing + R.Tal_crossing )- (L.Passing + L.Tal_Crossing );
    end
   ));
    MaxCrossing := lstBestCross[0].Passing + lstBestCross[0].Tal_Crossing  ;

    for P := lstBestCross.Count -1 downto 0 do begin
      if (lstBestCross[p].Passing + lstBestCross[0].Tal_Crossing) < MaxCrossing then
...
begin
  ...
  for I := lstSoccerReserve.Count -1 downto 0 do begin
    if (lstSoccerReserve[i].CellX = X) and (lstSoccerReserve[i].CellY = Y) and  (lstSoccerPlayer[i].team = Team)  then begin
...
function TSoccerBrain.GetSoccerPlayerInGame ( ids: string ): TSoccerPlayer;
var
  i: integer;
begin
  Result := nil;
  for I := lstSoccerPlayer.Count -1 downto 0 do begin
    if (lstSoccerPlayer[i].Ids = ids ) and ( not lstSoccerPlayer[i].Gameover )then begin
      Result := lstSoccerPlayer[i];
      exit;
    end;
  end;
...
begin
    Result := nil;
    for I := lstSoccerPlayer.Count -1 downto 0  do begin
      if lstSoccerPlayer[i].Gameover then Continue;
...
begin
  ...
  for P := lstSoccerPlayer.Count -1 downto 0 do begin
    ...
    if (absDistance( aPlayer.CellX ,aPlayer.CellY,  aFriend.CellX, aFriend.CellY ) > ( CrossingRangeMax  + aPlayer.tal_longpass ))
...
begin
   ...
   //CreateFormationCells;

//  dir_log:='c:\temp\';
  lstSoccerPlayer := TObjectList<TSoccerPlayer>.create(true);
...
//  if not (csDesigning in ComponentState) then begin
 ...
 //   TVCrossingAreaCells.Free;
   // lstSoccerPlayer.Free ;
    ...
    lstSoccerPlayer.Free;
    for I := lstSoccerReserve.Count -1 downto 0 do begin
     // lstSoccerReserve[i].Destroy;
      lstSoccerReserve[i].Free;
    end;
...
procedure TSoccerbrain.TVCreateCrossingAreaCells;
var
...
procedure TSoccerbrain.AICreateCrossingAreaCells;
var
...
procedure TSoccerbrain.CreateShotCells;
var
  aShotCell : TShotCell;
  aPoint: TPoint;
  aPpPoint: PPointL;
...
//    aShotCell.subCell.Add(@aPoint);
    ...
    for i :=  ShotCells.count -1 downto 0  do begin
      ...
      for ii := 0 to ShotCells[i].subCell.Count -1 do begin
        ...
        aPPpoint := ShotCells[i].subCell[ii];
        case apPPoint.X of
          0: aPoint.X := 11;
          1: aPoint.X := 10;
          2: aPoint.X := 9;
...
begin
  ...
  // per ogni calciatore avversario che si trovano a distanza < 2 dalla palla, prendo tutte le celle libere in cui può andare
    ...
    if AbsDistance(anOpponent.CellX, anOpponent.CellY, ball.CellX, ball.CellY ) < 2 then begin
      ...
      aRnd := RndGenerate0(aCellList.Count-1);
      TsScript.add ('sc_player,'+ anOpponent.Ids +','+IntTostr(anOpponent.CellX)+','+ IntTostr(anOpponent.CellY)+','+
...
var
  P,teamFault,Isfault: integer;
  label noCheckCheat;
begin

   FTeamMovesLeft := value;
   if GameStarted then begin
      TsScript.add ('sc_TML,' + IntTostr(FTeamMovesLeft) + ',' + IntTostr(TeamTurn) ) ;
      Minute := Minute + 1;
...
begin
    ...
    if isCheatingBallGK ( TeamTurn ) then begin // se quel portiere ha la palla. qui è dove sta per cambiare il turno ma ancora è il suo
      { fallo del GK, rigore }
      if TeamTurn = 0 then TeamFaultFavour := 1 else TeamFaultFavour :=0;
      TsScript.add ('sc_fault.cheatballgk,' + intTostr(TeamFaultFavour) +',' + IntTostr(Ball.CellX) +','+IntTostr(Ball.CellY) ) ; // informo il client del fallo
      if TeamTurn = 0 then TeamTurn := 1 else TeamTurn :=0;
    ...
    if GameStarted and  FlagEndGame then begin
      TsScript.add ('sc_GAMEOVER,' + intTostr(TeamTurn) ) ;
      GameStarted:= False;
      Finished:= True;
      FinishedTime := GetTickCount;  { TODO -con the road : bug alle 23.59 }
...
var
  tsCmd: Tstringlist;
  BaseShot,BaseHeading,BaseHeadingFriend,BaseGK,aRnd,aRnd2,aRnd3,arnd4,preRoll, preRoll2,preRoll3,preRoll4,CellX,CellY,BonusDefenseHeading: integer;
...
//      TSoccerBrain(Cli.Brain).BrainInput ( IntToStr(Cli.GuidTeam) + ',' + ts[1] + ',' + ts[2] + ',' + ts[3]  );
    ...
    if (absDistance( Ball.CellX ,Ball.CellY,  CellX, CellY ) > (ShortPassRange + aPlayer.tal_longpass))
    ...
    ToEmptyCell:= 0;
    if aFriend = nil then ToEmptyCell := ToEmptyCellMalus
    else begin
    ...
    TsScript.add ('SERVER_SHP,' + aPlayer.ids + ',' + IntToStr(aPlayer.CellX) + ',' + IntToStr(aPlayer.CellY)  + ',' +tsCmd[1] + ',' + tsCmd[2] ) ;//  skillused  tentativo ) ;
    ...
    aPlayer.xpTal_PlayMaker := aPlayer.xpTal_PlayMaker + 1;
...
//                  if (AbsDistance(aPlayer.CellX,aPlayer.CellY, Ball.CellX ,  Ball.CellY  ) = 1) or (aPlayer.CellY = Ball.CellY) or (aPlayer.CellX = Ball.CellX) then
        ...
        for Y := 0 to lstIntercepts.count -1 do begin
          ...
          if ( lstIntercepts[Y].Cell.X = aPath[i].X) and (lstIntercepts[Y].Cell.Y = aPath[i].Y) then begin  // se questa cella
            ...
            aRnd2:=  Roll2.value + anIntercept.Defense + anIntercept.Tal_bulldog + ToEmptyCell -1 ;

            TsScript.add ( 'sc_DICE,' + IntTostr(CellX) + ',' + Inttostr(CellY) +','+ IntTostr(aRnd2) +',' + IntToStr(anIntercept.Defense )+
            ',Intercept,'+ anIntercept.ids+','+IntTostr(Roll2.value) + ',' + Roll2.fatigue +',' + IntToStr(anIntercept.Tal_bulldog + ToEmptyCell -1));
            anIntercept.xp_Defense:= anIntercept.xp_Defense + 1;
            anIntercept.xpTal_bulldog:= anIntercept.Tal_bulldog + 1;
            //arnd2:=arnd+1;
            if aRnd2 > aRnd then begin // passaggio ---> intercpet prende la palla
              ...
              0..1: begin // prende la palla ma rimbalza in avanti di 1
                     ...
                     TsScript.add ('sc_player.move.intercept,'+ anIntercept.Ids +','+IntTostr(oldPlayer.x)+','+ IntTostr(oldPlayer.Y)+','+ IntTostr(anIntercept.CellX)+','+ IntTostr(anIntercept.CellY) ) ;
...
//           aPath.count è la lunghezza del passaggio che qui non +è stato intercettato perchè sopra exit
           if (aPlayer.Tal_PlayMaker > 0) then begin
  ...
  //Roll 10 la palla termina esattamente a cellx,celly e un compagno vicino si posiziona sulla palla (lancio perfetto)
    ...
    if tsCmd[3] <> 'GKLOP' then begin
      if (absDistance( Ball.CellX ,Ball.CellY,  CellX, CellY ) > (LoftedPassRangeMax + aPlayer.Tal_longpass))
  ...
  //  Ball.Player := nil;
   ...
   // aPlayer.xp_passing := aPlayer.xp_passing + 1; giusto cosi'
    aPlayer.xpTal_longPass := aPlayer.xpTal_longPass + 1;

    if aPlayer.Team = aPlayer.Field then
      aPlayer.xpTal_PlayMaker := aPlayer.xpTal_PlayMaker + 1; {  solo il lop nella propria metacampo }

    TsScript.add ('sc_ST,' + aPlayer.ids +',' + IntToStr(cost_lop) ) ;
...
//Passaggio alto VS Colpo di testa
      ...
      case aRnd of
        1..5:begin // lop la palla cade in una cella adiacente casuale
...
//              if (AbsDistance(aPlayer.CellX,aPlayer.CellY, aCell.X, aCell.Y ) = 1) or (aPlayer.CellY = aCell.Y)  then
              ...
              tsSpeaker.Add( aPlayer.Surname +' effettua il passaggio alto sbagliato' );
                ...
                if (LstHeading.Count > 1) and  (GetSoccerPlayer (Ball.cellx, ball.celly) = nil )then begin
                  ...
                  TsScript.add ('sc_player,'+ anOpponent.Ids +','+IntTostr(oldPlayer.X)+','+ IntTostr(oldPlayer.Y)+','+
                                                      IntTostr(anOpponent.CellX)+','+ IntTostr(anOpponent.CellY)  ) ;

                end;
             end;
        6..9:begin // la palla cade nella cella del target, ma 1=soggetto a heading e rimbalzerà 2= se heading difensivo va a vuoto, si procede col ball.control del target
             ...
             // OldBall:= Point ( Ball.CellX , Ball.Celly );
              ...
              // lop su friend --------->>>  BALL.CONTROL
                    ...
                    case aRnd3 of  //freind non controlla la palla
                      1..5: begin
                             ...
                             tsSpeaker.Add(aFriend.Surname + ' controlla male');
                               ...
                               TsScript.add ('sc_lop.ballcontrol.bounce,' + aPlayer.Ids {Lop} + ','+ aFriend.ids{cella}
                                                                       + ',' + IntTostr(aPlayer.cellx)+',' + IntTostr(aPlayer.celly) {celle}
                                                                       + ',' + IntTostr(aFriend.cellx)+',' + IntTostr(aFriend.cellY)
                                                                       + ',' + IntTostr(Ball.cellx)+',' + IntTostr(ball.celly)); {celle}
                      ...
                      6..9: begin // non controlla ma finisce su eventuale cella vuota e la raggiunge
                      ...
                      10..MAX_LEVEL: begin  // controlla perfettamente
                               TsScript.add ('sc_lop.ballcontrol.ok10,' + aPlayer.Ids {Lop} + ','+ aFriend.ids{cella}
                                                                       + ',' + IntTostr(aPlayer.cellx)+',' + IntTostr(aPlayer.celly) {celle}
                                                                       + ',' + IntTostr(aFriend.cellx)+',' + IntTostr(aFriend.cellY));
          ...
          10..MAX_LEVEL:begin  // il passaggio è perfetto, nessun Ball.Control del target e nessun heading difensivo ma, se in shotcell, VOLLEY (tiro al volo)
                // nel caso di non Volley, ma riceve a centrocampo ad esempio, ci sono i buff di ballcontrol+1

                   TsScript.add ('sc_lop.ok10,' + aPlayer.Ids {Lop} + ','+ aFriend.ids{cella}
                                                                       + ',' + IntTostr(aPlayer.cellx)+',' + IntTostr(aPlayer.celly) {celle}
                                                                       + ',' + IntTostr(aFriend.cellx)+',' + IntTostr(aFriend.cellY));
                      Ball.CellS := Point (CellX, CellY);
...
//                    if (AbsDistance(aPlayer.CellX,aPlayer.CellY, aCell.X, aCell.Y ) = 1) or (aPlayer.CellY = aCell.Y) Or (aPlayer.CellX = aCell.X) then
             ...
             // Volley  ABSDISTANCE LONTANA minimo 3 e proviene da linea palla o +1
               if (aFriend.InShotCell) and (AbsDistance(aPlayer.CellX ,aPlayer.CellY,  CellX, CellY ) >= VolleyRangeMin ) then Begin
                  if  ( (aPlayer.Team = 0) and (aPlayer.CellX >= CellX) )  or  ( (aPlayer.Team = 1) and (aPlayer.CellX <= CellX) ) then begin
                    ...
                    preRoll2 := RndGenerate (dice);
                    Roll2 := AdjustFatigue (aFriend.Stamina , preRoll);
    ...
    {$ifDEF SetAlwaysGol} arnd2 := 10; {$endif}
              ...
              //                TsScript.add ('sc_pos.back.bounce,' + aPlayer.ids + ',' + anOpponent.ids +','
              //                                              + IntTostr(aPlayer.cellx)+',' + IntTostr(aPlayer.celly) + ','
              //                                              + IntTostr(anOpponent.cellx)+',' + IntTostr(anOpponent.celly) +','
              //                                              + IntTostr(Ball.cellX)+',' + IntTostr(Ball.cellY ));
                              TsScript.add ('sc_lop.back.swap.bounce,' + aPlayer.ids + ',' + anOpponent.ids +','
                                                            + IntTostr(aPlayer.cellx)+',' + IntTostr(aPlayer.celly) + ','
                                                            + IntTostr(anOpponent.cellx)+',' + IntTostr(anOpponent.celly) +','
                                                            + IntTostr(Ball.cellX)+',' + IntTostr(Ball.cellY ));
              ...
              //            goto palo;
                          if aRnd4 > aRnd2 then begin // power.Shot ---> il portiere para e c'è il rimbalzo
                            ...
                            // la palla, che ora è in possesso del portiere , rimbalza e finisce in posizione random che calcolo adesso
                              TsScript.add ('sc_lop.bounce.gk,' + aPlayer.ids + ',' + aGK.ids{sfidante} +','
                                                            + IntTostr(aPlayer.cellx)+',' + IntTostr(aPlayer.celly) + ','
              ...
              //aGK := GetOpponentGK ( aPlayer.team);
                                 ...
                                 TsScript.add ('sc_lop.gol,' + aPlayer.ids + ','+ aFriend.ids + ',' + aGK.ids +','
                                                            + IntTostr(aPlayer.cellx)+',' + IntTostr(aPlayer.celly) + ','
                                              + IntTostr(aFriend.cellx)+',' + IntTostr(aFriend.celly) + ','
                                                            + IntTostr(aGK.cellx)+',' + IntTostr(aGK.celly) +','
                                                            + IntTostr(aGK.cellX)+',' + IntTostr(aGK.cellY)  );
...
//      CompileHeadingList (aPlayer.Team{avversari di}, 1{MaxDistance}, CellX, CellY, LstHeading  );
      case aRnd of
        1..5:begin // la palla cade in una cella adiacente casuale la quale può contenere un avversario, una cella vuota o un compagno
              aCell:= GetRandomCell ( CellX, CellY, 1 , false ,true);
...
//              if (AbsDistance(aPlayer.CellX,aPlayer.CellY, aCell.X, aCell.Y ) = 1) or (aPlayer.CellY = aCell.Y) or (aPlayer.CellX = aCell.X) then
                ...
                if aGhost = nil then begin
                  ...
                  if (LstHeading.Count > 1) and  (GetSoccerPlayer (Ball.cellx, ball.celly) = nil )then begin
                    ...
                    TsScript.add ('sc_player,'+ anOpponent.Ids +','+IntTostr(oldPlayer.X)+','+ IntTostr(oldPlayer.Y)+','+
                      ...
                      case aRnd3 of  //friend non controlla la palla
                        1..5: begin
                               aCell:= GetRandomCell  (  Ball.CellX, Ball.CellY , 1 , false ,true);
                               Ball.Cells := aCell;
                               aPossibleOffside := GetSoccerPlayer(Ball.CellX , ball.cellY );
                      ...
                      case aRnd3 of  //opponent non controlla la palla
                        1..5: begin

                                aCell:= GetRandomCell  ( Ball.CellX, Ball.CellY , 1, false  ,true);
                               Ball.Cells := aCell;
                                TsScript.add ('sc_lop.ballcontrol.bounce,' + aPlayer.Ids {Lop} + ','+ anOpponent.ids{cella}
                                                                       + ',' + IntTostr(aPlayer.cellx)+',' + IntTostr(aPlayer.celly) {celle}
                                                                       + ',' + IntTostr(anOpponent.cellx)+',' + IntTostr(anOpponent.cellY)
                                                                       + ',' + IntTostr(Ball.cellx)+',' + IntTostr(ball.celly)); {celle}
                        ...
                        6..9: begin // controlla ma finisce su eventuale cella vuota e la raggiunge
                               aCell:= GetRandomCell  ( Ball.CellX, Ball.CellY , 1, false  ,true); {  possibile cell=nil }
                                 ...
                                 TsScript.add ('sc_bounce,'+ IntTostr(anOpponent.CellX)+','+ IntTostr(anOpponent.CellY)+','+ IntTostr(aCell.X)+','+ IntTostr(aCell.Y) +',0' ) ;
                                 if GetSoccerPlayer (ball.CellX , ball.celly) = nil then begin
                        ...
                        10..MAX_LEVEL: begin  // controlla perfettamente
                                  TsScript.add ('sc_lop.ballcontrol.ok10,' + aPlayer.Ids {Lop} + ','+ anOpponent.ids{cella}
                                                                       + ',' + IntTostr(aPlayer.cellx)+',' + IntTostr(aPlayer.celly) {celle}
                                                                       + ',' + IntTostr(anOpponent.cellx)+',' + IntTostr(anOpponent.cellY));
        ...
        6..MAX_LEVEL:begin // la palla cade nella cella scelta  e non succede nulla altro
...
//
    ...
    if not aPlayer.CanSkill then begin
     ...
     goto myexit; // hack
      ...
      if (absDistance( Ball.CellX ,Ball.CellY,  CellX, CellY ) > (CrossingRangeMax + aPlayer.tal_longpass))
    ...
    TsScript.add ('SERVER_CRO,' + aPlayer.Ids + ',' + IntToStr(aPlayer.CellX) + ',' + IntToStr(aPlayer.CellY) +',' +IntTostr(CellX)+','+IntToStr(CellY) ) ;
    aPlayer.Stamina := aPlayer.Stamina - cost_cro;
    TsScript.add ('sc_ST,' + aPlayer.ids +',' + IntToStr(cost_cro) ) ;
    ExceptPlayers.Add(aPlayer);
    ...
    preRoll := RndGenerate (dice);
    Roll := AdjustFatigue (aPlayer.Stamina , preRoll);
    aRnd:= Roll.value + aPlayer.Passing + aPlayer.Tal_Crossing ; // se undepressure 0   il talento gli conferisce 1
    aPlayer.resetALL;
    aPlayer.xpTal_Crossing := aPlayer.xpTal_Crossing + 1;
    TsScript.add ( 'sc_DICE,' + IntTostr(aPlayer.CellX) + ',' + Inttostr(aPlayer.CellY) +','+  IntTostr(aRnd) + ','+
    IntToStr(aPlayer.Passing )+',Crossing,'+ aPlayer.ids+','+IntTostr(Roll.value)+','+Roll.fatigue+','+IntToStr(aPlayer.Tal_Crossing)  );
    ...
    CompileHeadingList (aPlayer.Team{avversari di}, 1{MaxDistance}, CellX, CellY, LstHeading  );
    case aRnd of
      1..5:begin // cross errore la palla cade in una cella adiacente casuale
            aCell:= GetRandomCell ( CellX, CellY, 1 , false ,true);
            Ball.Cells := aCell;
...
//HVSH:
  ...
  //  ---------->>>      avversario     heading automatico in respinta di 2
      ...
      6..9:begin // la palla termina esattamente sul friend a cellx,celly . ci sono i bonus della difesa che dipendono dalla posizione e heading vs heading
      ...
      10..MAX_LEVEL:begin //Roll 10 la palla termina esattamente sul friend a cellx,celly e il compagno ha +1 in heading (lancio perfetto). la difesa ha comunque i suoi bonus
           //HVSH con certi bonus
              aCell.X := CellX;
              aCell.Y := CellY;
              Ball.Cells := aCell;
...
//                if Roll3 >= Roll2 then begin
              ...
              aGK.xpTal_GoalKeeper := aGK.xpTal_GoalKeeper + 1;
              //              TsScript.add ('gkdive,'+ anOpponent.Ids ) ;
...
// GOL
                 ...
                 // TsScript.add ('sc_bounce.heading,'+ aHeadingFriend.ids +',' +IntTostr(Ball.CellX)+','+ IntTostr(Ball.CellY)+','+  IntTostr(aGK.CellX)+','+ IntTostr(aGK.CellY) +',1' ) ;
...
//aGK := GetOpponentGK ( aHeadingFriend.Team );
  ...
  // dribbling
    ...
    End;
      ...
      OldCell := aPlayer.Cells;
       ...
       TsScript.add ('SERVER_DRI,' + aPlayer.ids  + ',' + IntToStr(aPlayer.CellX) + ',' + IntToStr(aPlayer.CellY) + ',' +tsCmd[1] + ',' + tsCmd[2] ) ;//  skillused  tentativo) ;
       aPlayer.Stamina := aPlayer.Stamina - cost_dri;
       TsScript.add ('sc_ST,' + aPlayer.ids +',' + IntToStr(cost_dri) ) ;
       aPlayer.xpTal_Dribbling :=  aPlayer.xpTal_Dribbling + 1;
       ExceptPlayers.Add(aPlayer);
       preRoll := RndGenerate (dice);
       Roll := AdjustFatigue (aPlayer.Stamina , preRoll);
      ...
      if ( aRnd >= aRnd2 )  then begin // dribbling ---> player riesce nel dribbling
        ...
        if( (aRnd-aRnd2) >= DribblingDiff ) and (dstCell.X <> -1)  then begin // si sposta in avanti

            TsScript.add ('sc_dribbling.ok10,' + aPlayer.ids{sfidante} +',' + anOpponent.ids {cella}
                                              + ',' + IntTostr(aPlayer.CellX)+',' + IntTostr(aPlayer.CellY)
                                              + ',' + IntTostr(anOpponent.cellx)+',' + IntTostr(anOpponent.cellY)
            ...
            TsScript.add ('sc_dribbling.ok,' + aPlayer.ids{sfidante} +',' + anOpponent.ids {cella}
                                              + ',' + IntTostr(aPlayer.CellX)+',' + IntTostr(aPlayer.CellY)
                                              + ',' + IntTostr(anOpponent.cellx)+',' + IntTostr(anOpponent.cellY)
                                              + ',' + IntTostr(anOpponent.cellx)+',' + IntTostr(anOpponent.cellY)  ) ;
          SwapPlayers ( aPlayer, anOpponent );
          ...
          tsSpeaker.Add( aPlayer.Surname +' (Dribbling) vince il dribbling su ' + anOpponent.Surname );
            ...
            TsScript.add ('sc_dribbling.no,' + aPlayer.ids{sfidante} +',' + anOpponent.ids {cella}
                                              + ',' + IntTostr(aPlayer.CellX)+',' + IntTostr(aPlayer.CellY)
                                              + ',' + IntTostr(anOpponent.cellx)+',' + IntTostr(anOpponent.cellY)
                                              + ',' + IntTostr(aPlayer.cellx)+',' + IntTostr(aPlayer.cellY)  ) ;
  ...
  // Power shot
    ...
    if w_FreeKick3 then begin
      BaseShot :=  aPlayer.Shot + (MalusPowerShot[aPlayer.CellX]+1) + aPlayer.Tal_freekicks ; // punizione bonus +1
      TsScript.add ('SERVER_POS3,' + aPlayer.ids + ',' + IntToStr(aPlayer.CellX) + ',' + IntToStr(aPlayer.CellY) + ',' + IntToStr( aDoor.X) +',' + IntToStr( aDoor.Y));
      Kind := '.golpos3.';
      Modifier :=  MalusPowerShot[aPlayer.CellX]+1 + aPlayer.Tal_freekicks;
    end
      ...
      Kind := '.golpos4.';
      Modifier :=  modifier_penalty + 2;
    end
    else  begin
      BaseShot :=  aPlayer.Shot + MalusPowerShot[aPlayer.CellX]; // tiro normale
      TsScript.add ('SERVER_POS,' + aPlayer.ids + ',' + IntToStr(aPlayer.CellX) + ',' + IntToStr(aPlayer.CellY)+ ',' + IntToStr( aDoor.X) +',' + IntToStr( aDoor.Y));
      Kind := '.golpos.';
      Modifier :=  MalusPowerShot[aPlayer.CellX];
    end;
    if BaseShot <= 0 then BaseShot := 1;
    Ball.Player.xpTal_Bomb := Ball.Player.xpTal_Bomb + 1;
    ...
    if w_FreeKick3 then begin
      ...
      TeamFreeKick := -1;
      aPlayer.isFK3  := false;
      aPlayer.xpTal_freekicks :=  aPlayer.xpTal_freekicks +1;     // 1 punto aggountivo a quello di default
        ...
        if (anOpponent.CellX = aCellBarrier.X)  and (anOpponent.CellY = aCellBarrier.Y) then begin
         ...
         // oldball:= Point ( anOpponent.CellX, anOpponent.CellY);
          if aRnd2 > aRnd then begin
            ...
            // il tiro raggiunge la cella e rimbalza
            TsScript.add ('sc_pos.bounce,' + aPlayer.ids + ',' + anOpponent.ids +','
                                            + IntTostr(aPlayer.cellx)+',' + IntTostr(aPlayer.celly) + ','
                                            + IntTostr(anOpponent.cellx)+',' + IntTostr(anOpponent.celly) +','
                                            + IntTostr(Ball.cellX)+',' + IntTostr(Ball.cellY ));
    ...
    {$ifDEF Setposprscorner} goto posvsgk; {$endif}
...
//                                              + IntTostr(anOpponent.cellx)+',' + IntTostr(anOpponent.celly) +','
//                                              + IntTostr(Ball.cellX)+',' + IntTostr(Ball.cellY ));
                TsScript.add ('sc_pos.back.swap.bounce,' + aPlayer.ids + ',' + anOpponent.ids +','
                                              + IntTostr(aPlayer.cellx)+',' + IntTostr(aPlayer.celly) + ','
                                              + IntTostr(anOpponent.cellx)+',' + IntTostr(anOpponent.celly) +','
                                              + IntTostr(Ball.cellX)+',' + IntTostr(Ball.cellY ));
                SwapPlayers (aPlayer,anOpponent );
...
//            goto palo;
    {$ifDEF Setposprscorner} aRnd2:=12; {$endif}
            if aRnd2 > aRnd then begin // power.Shot ---> il portiere para e c'è il rimbalzo
               ...
               Ball.Cells := aCell;
              // la palla, che ora è in possesso del portiere , rimbalza e finisce in posizione random che calcolo adesso
                TsScript.add ('sc_pos.bounce.gk,' + aPlayer.ids + ',' + aGK.ids{sfidante} +','
                                              + IntTostr(aPlayer.cellx)+',' + IntTostr(aPlayer.celly) + ','
                                              + IntTostr(aGK.cellx)+',' + IntTostr(aGK.celly)  +','
...
//                    aPlayer.resetALL;
                    ...
                    TsScript.add ('sc_pos.gol,' + aPlayer.ids + ',' + aGK.ids{sfidante} +','
                                              + IntTostr(aPlayer.cellx)+',' + IntTostr(aPlayer.celly) + ','
                                              + IntTostr(aGK.cellx)+',' + IntTostr(aGK.celly) +','
  ...
  // non è uguale a pos. rimbalzo a e non a 2
    ...
    Roll := AdjustFatigue (aPlayer.Stamina , preRoll);
    if w_FreeKick3 then begin
      BaseShot :=  aPlayer.Shot + (MalusPrecisionShot[aPlayer.CellX]+1)  + aPlayer.Tal_freekicks ; // punizione bonus -1 di base
      Kind := '.golprs3.';
      Modifier :=  MalusPrecisionShot[aPlayer.CellX]+1 + aPlayer.Tal_freekicks;
    end
    else if w_FreeKick4 then begin
      BaseShot :=  aPlayer.Shot + modifier_penalty + 3; // rigore prs
      Kind := '.golprs4.';
      Modifier :=  modifier_penalty + 3;
    end
    else begin
      ...
      for I := lstSoccerPlayer.Count -1 downto 0 do begin
        ...
        if (anOpponent.CellX = aCellBarrier.X)  and (anOpponent.CellY = aCellBarrier.Y) then begin
          ...
          if aRnd2 > aRnd then begin
            ...
            TsScript.add ('sc_prs.stealball,' + aPlayer.ids + ',' + anOpponent.ids{sfidante} +','
                                          + IntTostr(aPlayer.cellx)+',' + IntTostr(aPlayer.celly) + ','
                                          + IntTostr(anOpponent.cellx)+',' + IntTostr(anOpponent.celly) +','
                                          + IntTostr(Ball.cellX)+',' + IntTostr(Ball.cellY) );
...
//      aPlayer := GetFK4 ;
      ...
      // ShotCells
      //
    {$ifDEF Setposprscorner} goto prsvsgk; {$endif}
        for ii := 0 to ShotCells.Count -1 do begin
          // la direttiva principale
          if (ShotCells[ii].DoorTeam <> aPlayer.Team) and
            (ShotCells[ii].CellX = aPlayer.CellX) and (ShotCells[ii].CellY = aPlayer.CellY) then begin
            ...
            ExceptPlayers.Add(anOpponent);
              ...
              if aRnd2 > aRnd then begin // precision.Shot ---> avversario prende la palla
               ...
               // Assegno la nuova posizione  della palla
                ...
                TsScript.add ('sc_prs.back.stealball,' + aPlayer.ids + ',' + anOpponent.ids{sfidante} +','
                                              + IntTostr(aPlayer.cellx)+',' + IntTostr(aPlayer.celly) + ','
                                              + IntTostr(anOpponent.cellx)+',' + IntTostr(anOpponent.celly)+','
                                              + IntTostr(Ball.cellX)+',' + IntTostr(Ball.cellY) )
                else
...
reason:='';
                    ...
                    TsScript.add ('sc_prs.gol,' + aPlayer.ids + ',' + aGK.ids{sfidante} +','
                                              + IntTostr(aPlayer.cellx)+',' + IntTostr(aPlayer.celly) + ','
                                              + IntTostr(aGK.cellx)+',' + IntTostr(aGK.celly) +','
                                              + IntTostr(aGK.cellX)+',' + IntTostr(aGK.cellY)+ ',' +IntTostr(RndGenerate(2)) );
                    aPlayer.resetPRE ;  // eventuale pressing viene perso DOPO il tiro
...
reason:='';
    ...
    aPlayer.xpTal_Experience := aPlayer.xpTal_Experience + 1;
    ExceptPlayers.Add(aPlayer);
    ...
    (Ball.Player.team <> aPlayer.Team ) then begin
     ...
     // if Ball.Player.Defense < 0 then Ball.Player.Defense :=0;
      ...
      if aPlayer.Tal_Experience = 0 then begin
        TeamMovesLeft := TeamMovesLeft - 1;
...
reason:='';
    ...
    TsScript.add ('SERVER_PRO,' + aPlayer.ids + ',' + IntToStr(aPlayer.cellX) + ',' + IntToStr(aPlayer.cellY)+ ',' + IntToStr(aPlayer.cellX) + ',' + IntToStr(aPlayer.cellY) ) ;
    aPlayer.Stamina := aPlayer.Stamina - cost_pro;
    TsScript.add ('ST,' + aPlayer.ids +',' + IntToStr(cost_pro) ) ;
    aPlayer.xpTal_Power :=  aPlayer.xpTal_Power + 1;
    ExceptPlayers.Add(aPlayer);
    aPlayer.BonusProtectionTurn := 2;
    aPlayer.BallControl := aPlayer.BallControl + 2;
    aPlayer.CanMove := false;
    aPlayer.CanSkill := false;
    TeamMovesLeft := TeamMovesLeft - 1;
    if TeamMovesLeft <= 0 then TurnChange  (TurnMoves);
    ...
    aPlayer := GetSoccerPlayerInGame ( tsCmd[1]); { nel caso di un espulso o sostituito }
    if aPlayer = nil then begin
...
// attuale. Effettuare un Contrasto da dietro alza moltissimo la probabilità di fare fallo con possibili infortuni e/o cartellini.
    ...
    TsScript.Add('SERVER_TAC,' + aPlayer.ids + ',' + IntToStr(aPlayer.CellX) + ',' + IntToStr(aPlayer.CellY)+ ',' + IntToStr(Ball.Player.CellX) + ',' + IntToStr(Ball.Player.CellY) ) ;
    ...
    case Isfault of
      0:Begin   // non è fallo
        TeamMovesLeft := TeamMovesLeft - 1;
        if TeamMovesLeft <= 0 then TurnChange (TurnMoves);// else TsScript.add ('E') ;
        AI_MoveAll;
        TsScript.add ('E');
        reason:='';
        goto MyExit;
...
//        MoveInDefaultField(aPlayer);
    ...
    TsScript.add ('SERVER_TACTIC,' + aPlayer.ids + ',' + IntToStr(aPlayer.defaultCellX) + ',' + IntToStr(aPlayer.defaultCellY) + ',' + tsCmd[2] + ',' + tsCmd[3]) ;
    aPlayer.DefaultCells := Point(CellX,CellY);

    TsScript.add ('sc_tactic,' + aPlayer.ids + ',' + IntToStr(aPlayer.defaultCellX) + ',' + IntToStr(aPlayer.defaultCellY) + ',' + tsCmd[2] + ',' + tsCmd[3]) ;
    TsScript.add ('sc_TML,' + IntTostr(FTeamMovesLeft) + ',' + IntTostr(TeamTurn) ) ;    // non incremento i minuti ma le mosse decrementano
...
//        aPlayer.DefaultCellS  := point (CellX,CellY);
//        MoveInDefaultField(aPlayer);
    aPlayer := GetSoccerPlayer2 ( tsCmd[1]);
    if aPlayer = nil then begin
...
{    if ((CellX = 0) or (CellX = 2)  or  (CellX = 5) or (CellX = 8)) and (aPlayer.Team <> 0) then Begin
    ...
    TsScript.add ('SERVER_SUB,' + aPlayer.ids + ',' +  aPlayer2.ids);
    ...
    TsScript.add ('sc_sub,' + aPlayer.ids  + ',' + aPlayer2.ids) ;

    fTeamMovesLeft := fTeamMovesLeft -1 ;
    TsScript.add ('sc_TML,' + IntTostr(FTeamMovesLeft) + ',' + IntTostr(TeamTurn) ) ;    // non incremento i minuti
//    TeamMovesLeft := TeamMovesLeft -1 ;
    if TeamMovesLeft <= 0 then TurnChange (TurnMoves);// else TsScript.add ('E') ;
    TsScript.add ('E');
    reason:='';
    ...
    if  aPlayer.HasBall then begin
      ...
      aPlayer.xpTal_Positioning := aPlayer.xpTal_Positioning + 1; // solo senza palla
    ...
    for I := 0 to aPlayer.MovePath.Count -1 do begin
       ...
       if MyParam = withball then begin //aPlayer.HasBall then begin   se il player AVEVA la palla
       ...
       // se NON ha la palla
          ...
          TsScript.add ('sc_player,'+ aPlayer.Ids +','+IntTostr(aPlayer.CellX)+','+ IntTostr(aPlayer.CellY)+','+
          IntTostr(aPlayer.MovePath [i].X)+','+ IntTostr(aPlayer.MovePath [i].Y)  ) ;
    ...
    case aPlayer.Team of
      0: begin
        if aPlayer.CellX <= 5 then
          aPlayer.xpTal_midDefensive := aPlayer.xpTal_midDefensive + 1
          else aPlayer.xpTal_midOffensive := aPlayer.xpTal_midOffensive + 1;
      end;
      1: begin
...
// input dal client setto chi calcia il corner dopo la richiesta SERVER_COA del brain tutto da validare
      ...
      // molto importante la sequenza
         ...
         if swapPlayer <> nil then begin
           if (swapPlayer.Team = aPlayer.Team) and (SwapPlayer.Ids <> aPlayer.ids) then begin    // swap compagno
              ...
              TsScript.add ('sc_player,'+ swapPlayer.Ids +','+IntTostr(swapPlayer.CellX)+','+ IntTostr(swapPlayer.CellY)+','+
       ...
       TsScript.add ('COA.IS,'+  tt +','+ tsCmd[1]  +',' + tsCmd[2] +',' + tsCmd[3] +',' + tsCmd[4] + ',' + SwapString.CommaText );
     ...
     SwapString.Add('0');
       ...
       if teamTurn = 0 then tt := '1' else tt :='0';
       TsScript.add ('COD.IS,' + tt + ',' + tsCmd[1] +',' + tsCmd[2] +',' + tsCmd[3]+','+SwapString.CommaText );
       Turnchange(TurnMoves);
       SwapString.Free;
       TsScript.add ('E' ) ;
       w_coa := false;
    ...
    if GetSoccerPlayer ( tsCmd[1], TeamTurn) = nil then begin
     ...
     if swapPlayer <> nil then begin
       ...
       TsScript.add ('FKA1.IS,'+ IntToStr(teamTurn) + ',' +tsCmd[1]  + ',' + SwapString.CommaText );
       //Turnchange(TurnMoves);
       SwapString.Free;
       TsScript.add ('E' ) ;
       w_fka1 := false;
    ...
    End;
     ...
     TsScript.add ('sc_player,'+ aPlayer.Ids +','+IntTostr(aPlayer.CellX)+','+ IntTostr(aPlayer.CellY)+','+
      ...
      // molto importante la sequenza
       ...
       if teamTurn = 0 then tt := '1' else tt :='0';
       TsScript.add ('FKA2.IS,'+  tt + ',' + tsCmd[1]  +',' + tsCmd[2] +',' + tsCmd[3] +',' + tsCmd[4] + ',' + SwapString.CommaText );
       Turnchange(TurnMoves);
       SwapString.Free;
       TsScript.add ('E' ) ;
       w_fka2 := false;
    ...
    if NOT w_Fkd2  then begin
     ...
     TsScript.add ('SERVER_FKD2.IS');
       ...
       if teamTurn = 0 then tt := '1' else tt :='0';
       TsScript.add ('FKD2.IS,' + tt + ',' + tsCmd[1] +',' + tsCmd[2] +',' + tsCmd[3]+','+SwapString.CommaText );
       SwapString.Free;
       Turnchange(TurnMoves);
       TsScript.add ('E' ) ;
       w_fka2 := false;
    ...
    if GetSoccerPlayer ( tsCmd[1], TeamTurn) = nil then begin
     ...
     //     swapPlayer.Cells := aCell2;
       ...
       if teamTurn = 0 then tt := '1' else tt :='0';
       TsScript.add ('FKA3.IS,'+ tt +',' +tsCmd[1]  + ',' + SwapString.CommaText );
       Turnchange(TurnMoves);
       SwapString.Free;
       TsScript.add ('E' ) ;
       w_fka3 := false;
     ...
     SwapDone := False;
       ...
       for I := 0 to 3 do begin // 4 in barriera
         ...
         if (swapPlayer <> nil) and (Not SwapDone) and (i = 0) then begin   // evita swap del primo in barriera
           ...
           TsScript.add ('sc_player.barrier,'+ aPlayer.Ids +','+IntTostr(aPlayer.CellX)+','+ IntTostr(aPlayer.CellY)+','+
       ...
       if teamTurn = 0 then tt := '1' else tt :='0';
       TsScript.add ('FKD3.IS,' + tt +',' + tsCmd[1] +',' + tsCmd[2] +',' + tsCmd[3]+','+  tsCmd[4] + ',' + SwapString.CommaText );
       SwapString.Free;
       Turnchange(TurnMoves);
       TsScript.add ('E' ) ;
       w_fka3 := false;
...
//          swapPlayer.Cells := aCell2;
       ...
       TsScript.add ('FKA4.IS,'+ intTostr(TeamTurn) +','+ tsCmd[1]  + ',' + SwapString.CommaText );
    //   Turnchange(TurnMoves);
       SwapString.Free;
       TsScript.add ('E' ) ;
       w_fka4 := false;
...
{    SaveData (incMove );
    ...
    if aPlayer <> nil then begin
      aGK := GetOpponentGK ( aPlayer.team);
      TsScript.add ('SERVER_POS,' + aPlayer.ids + ',' + IntToStr(aPlayer.CellX) + ',' + IntToStr(aPlayer.CellY)+ ',' + IntToStr( aGK.CellX ) +',' + IntToStr(aGK.celly));
      CornerSetup ( aPlayer );
      goto Myexit;
...
//  MMbraindata.Write( @BEGINBRAIN[1], 10 );
  ...
  for i := 0 to lstSoccerPlayer.Count -1 do begin    // salvo tutti i player che stanno giocando o hanno giocato ( es. espulsi rimangono in questa lista )
    PlayerGuid := StrToInt(lstSoccerPlayer[i].Ids); // dipende dalla gestione players, se divido per nazioni?
...
begin
    ...
    (Ball.Player.team <> aPlayer.Team ) and( Ball.Player.Role <> 'G') then begin
      ...
      OldCell := aPlayer.Cells;
       ...
       preRoll := RndGenerate (dice);
       Roll := AdjustFatigue (aPlayer.Stamina , preRoll);
       aRnd:=  Roll.value + aPlayer.Defense + aPlayer.tal_toughness;  // durezza
       aPlayer.xp_defense := aPlayer.xp_Defense + 1;
       aPlayer.xpTal_Toughness := aPlayer.xpTal_Toughness + 1;
       aPlayer.xpTal_faul := aPlayer.xpTal_faul + 1;
       aPlayer.xpTal_Marking := aPlayer.xpTal_Marking + 1;

       preRoll2 := RndGenerate (dice);
       Roll2 := AdjustFatigue (Ball.Player.Stamina , preRoll2);
       aRnd2:= Roll2.value + Ball.Player.ballControl + Ball.Player.Tal_Power ;  // se a 0 perchè underpressure, il talento gli conferisce minimo 1
       Ball.Player.xp_ballControl:= Ball.Player.xp_ballControl + 1;
       Ball.Player.xpTal_Power := Ball.Player.xpTal_Power + 1;
       if aRnd2 < 0 then aRnd2 :=0;
       ExceptPlayers.Add(ball.player);

        TsScript.add ( 'sc_DICE,' + IntTostr(aPlayer.CellX) + ',' + Inttostr(aPlayer.CellY) +','+  IntTostr(aRnd) +','+
        IntTostr (aPlayer.Defense) + ',Tackle,'+ aPlayer.ids+','+IntTostr(Roll.value) + ','+ Roll.fatigue +','+IntToStr(aPlayer.tal_toughness));
...
//        aRnd2 := 10;
     ...
     //   if (( aRnd >= aRnd2 ) and ( (aRnd-aRnd2) < TackleDiff )) or (( (aRnd-aRnd2) >= TackleDiff ) and (dstCell.X = -1)) then begin
        ...
        If ( TackleResult >= TackleDiff ) and (dstCell.X <> -1)  Then begin
          ...
          TsScript.add ('sc_tackle.ok10,' + aPlayer.ids{sfidante} +',' + oldPlayerBall.ids {cella}
                                      + ',' + IntTostr(aPlayer.CellX)+',' + IntTostr(aPlayer.CellY)
...
//        else If ( (aRnd-aRnd2) >= TackleDiff ) and (dstCell.X <> -1)  Then begin
          ...
          TsScript.add ('sc_tackle.ok,' + aPlayer.ids{sfidante} +',' + oldPlayerBall.ids {cella}
                                      + ',' + IntTostr(aPlayer.CellX)+',' + IntTostr(aPlayer.CellY)
                                      + ',' + IntTostr(Ball.Player.CellX)+',' + IntTostr(Ball.Player.CellY)
                                      + ',' + IntTostr(Ball.Player.CellX)+',' + IntTostr(Ball.Player.CellY)   ) ;
       ...
       // non resetto PROPRE
          ...
          if aPlayer.Tal_faul > 0 then fault := fault + 15;
          aPlayer.CanSkill := False;
...
begin
    ...
    (Ball.Player.team <> aPlayer.Team ) then begin
      ...
      OldCell := aPlayer.Cells;
       ...
       TsScript.add ('sc_ST,' + aPlayer.ids +',' + IntToStr(cost_autotac) ) ;
       preRoll := RndGenerate (dice);
       Roll := AdjustFatigue (aPlayer.Stamina , preRoll);
       aRnd:=  Roll.value + aPlayer.Defense +  aPlayer.Tal_Challenge ; // lottatore
       aPlayer.xp_defense := aPlayer.xp_Defense + 1;
       aPlayer.xpTal_Challenge :=  aPlayer.xpTal_Challenge +1;
       aPlayer.xpTal_Marking := aPlayer.xpTal_Marking + 1;

       preRoll2 := RndGenerate (dice);
       Roll2 := AdjustFatigue (Ball.Player.Stamina , preRoll2);
       aRnd2:= Roll2.value + Ball.Player.ballControl + Ball.Player.tal_Power ; //  resiste aigli autotackle e ai tackle  toughness
       Ball.Player.xp_ballControl:= Ball.Player.xp_ballControl + 1;
       Ball.Player.xptal_Power :=  Ball.Player.xptal_Power +1;
       ...
       if ( aRnd > aRnd2 )  then begin // Tackle ---> player prende la palla
        ...
        oldPlayerBall := Ball.Player;
        TsScript.add ( 'sc_DICE,' + IntTostr(aPlayer.CellX) + ',' + Inttostr(aPlayer.CellY) +','+  IntTostr(aRnd) +','+
        IntTostr ( aPlayer.Defense )+',Tackle,'+ aPlayer.ids+','+IntTostr(Roll.value)+','+Roll.fatigue+','+IntToStr(aPlayer.Tal_Challenge));
        TsScript.add ( 'sc_DICE,' + IntTostr(oldPlayerBall.CellX) + ',' + Inttostr(oldPlayerBall.CellY) +','+  IntTostr(aRnd2) +','+
        IntTostr(oldPlayerBall.ballControl) +',Ball.Control,'+  oldPlayerBall.ids+','+IntTostr(Roll2.value)+','+Roll2.fatigue+','+IntToStr(oldPlayerBall.tal_Power));
   ...
   case Team of
    0: begin
      if Y < 3 then begin
        ...
        Result.GK.X := 11;
        Result.GK.Y := 3;
        Result.CornerCellOffset.X := +20;
        Result.CornerCellOffset.Y := -20;
...
begin
  ...
  // informo il client
  TsScript.add ('SERVER_COR,' + aplayer.ids + ','+ IntTostr(aPlayer.CellX) +',' + inttostr(aPlayer.CellY) + ',' +
                                                IntTostr(CornerMap.HeadingCellA [0].X) + ',' + IntTostr(CornerMap.HeadingCellA [0].Y)) ;
  aPlayer.Stamina := aPlayer.Stamina - cost_cor;
  TsScript.add ('ST,' + aPlayer.ids +',' + IntToStr(cost_cor) ) ;

  preRoll := RndGenerate (dice);
  Roll := AdjustFatigue(aPlayer.Stamina,PreRoll);
  aRnd:= aPlayer.Passing + aPlayer.Tal_Crossing  + Roll.value;
...
begin
  ...
  TsScript.add ('SERVER_CRO,' + aplayer.ids + ','+ IntTostr(aPlayer.CellX) +',' + inttostr(aPlayer.CellY) + ',' +
                                                IntTostr(CornerMap.HeadingCellA [0].X) + ',' + IntTostr(CornerMap.HeadingCellA [0].Y)) ;
  aPlayer.Stamina := aPlayer.Stamina - cost_cor;
  TsScript.add ('ST,' + aPlayer.ids +',' + IntToStr(cost_cor) ) ;   // costa come il corner
                ...
                if aRnd2 > Arnd3 then begin   //  heading difensivo vince
                  tsSpeaker.Add(aHeadingOpponent.Surname +' respinge di testa' );
                   ...
                   if (Ball.CellX = 2) or (ball.CellX = 9) then begin
                    ...
                    //bounce
                     Ball.Cells:= GetBounceCell  ( aPlayer.CellX,aPlayer.CellY, aHeadingOpponent.CellX, aHeadingOpponent.CellY, 2 , aHeadingOpponent.team);
                     TsScript.add ('sc_cro2.headingdef.swap.bounce,' + aPlayer.Ids +',' + aHeadingFriend.ids + ',' + aHeadingOpponent.ids
                                                               + ',' + IntTostr(CornerMap.CornerCell.X)+','+ IntTostr(CornerMap.CornerCell.Y)
                                                               + ',' + IntTostr(aPlayer.cellx)+',' + IntTostr(aPlayer.celly) {celle}
                                                               + ',' + IntTostr(aHeadingFriend.cellx)+',' + IntTostr(aHeadingFriend.celly) {celle}
                                                               + ',' + IntTostr(aHeadingOpponent.cellx)+',' + IntTostr(aHeadingOpponent.celly)
                     ...
                     Ball.Cells:= GetBounceCell  ( aPlayer.CellX,aPlayer.CellY, aHeadingOpponent.CellX, aHeadingOpponent.CellY, 2 , aHeadingOpponent.team);
                     TsScript.add ('sc_cro2.headingdef.bounce,'  + aPlayer.Ids +',' + aHeadingFriend.ids + ',' + aHeadingOpponent.ids
                                                               + ',' + IntTostr(CornerMap.CornerCell.X)+','+ IntTostr(CornerMap.CornerCell.Y)
                                                               + ',' + IntTostr(aPlayer.cellx)+',' + IntTostr(aPlayer.celly) {celle}
                                                               + ',' + IntTostr(aHeadingOpponent.cellx)+',' + IntTostr(aHeadingOpponent.celly)
                                                               + ',' + IntTostr(Ball.cellx)+',' + IntTostr(Ball.celly));
...
// se si arriva qui heading offensivo corner vince
         ...
         if (Ball.CellX = 1) or (ball.CellX = 10) then begin
           OldCell := aHeadingFriend.Cells ;
           SwapPlayers ( aHeadingFriend,aHeadingOpponent );
                       TsScript.add ('sc_cro2.headingatt.swap,' + aHeadingFriend.ids + ',' + aHeadingOpponent.ids
                                                               + ',' + IntTostr(aHeadingFriend.cellx)+',' + IntTostr(aHeadingFriend.celly)
                                                               + ',' + IntTostr(aHeadingOpponent.cellx)+',' + IntTostr(aHeadingOpponent.celly));
             ...
             // aRnd4:= 10;
              ...
              if aRnd4 > aRnd3 then begin // heading ---> il portiere para e c'è il rimbalzo
                ...
                // Ball.CellS := Point(0,1);
                 tsSpeaker.Add(aGK.Surname +' para e respinge');
                 TsScript.add ('sc_cro2.bounce.gk,' + aPlayer.ids + ','+ aHeadingFriend.ids + ',' + aGK.ids +','
                                              + IntTostr(CornerMap.CornerCell.X)+','+ IntTostr(CornerMap.CornerCell.Y) +','
                                              + IntTostr(aPlayer.cellx)+',' + IntTostr(aPlayer.celly) + ','
                                              + IntTostr(aHeadingFriend.cellx)+',' + IntTostr(aHeadingFriend.celly) + ','
                                              + IntTostr(aGK.cellx)+',' + IntTostr(aGK.celly)  +','
...
cor_crossbar:
                  ...
                  // se il portiere è fuori dai pali, la palla può rimbalzare in gol più sotto perchè il GK usa defense
                   ...
                   TsScript.add ('sc_cro2.gol,' + aPlayer.ids + ','+ aHeadingFriend.ids + ',' + aGK.ids +','
                                              + IntTostr(CornerMap.CornerCell.X)+','+ IntTostr(CornerMap.CornerCell.Y)+','
                                              + IntTostr(aPlayer.cellx)+',' + IntTostr(aPlayer.celly) + ','
                                              + IntTostr(aHeadingFriend.cellx)+',' + IntTostr(aHeadingFriend.celly) + ','
                                              + IntTostr(aGK.cellx)+',' + IntTostr(aGK.celly)  +','
...
procedure TSoccerbrain.createReserveSlotTV ;
begin
    reserveSlotTV[0,-4,0] := Point(-2,0);
    reserveSlotTV[0,-4,1] := Point(-2,1);
    reserveSlotTV[0,-4,2] := Point(-2,2);
    reserveSlotTV[0,-4,3] := Point(-2,3);
    reserveSlotTV[0,-4,4] := Point(-2,4);
    reserveSlotTV[0,-4,5] := Point(-2,5);
    reserveSlotTV[0,-4,6] := Point(-2,6);

    reserveSlotTV[0,-3,0] := Point(-1,0);
    reserveSlotTV[0,-3,1] := Point(-1,1);
    reserveSlotTV[0,-3,2] := Point(-1,2);
    reserveSlotTV[0,-3,3] := Point(-1,3);
    reserveSlotTV[0,-3,4] := Point(-1,4);
    reserveSlotTV[0,-3,5] := Point(-1,5);
    reserveSlotTV[0,-3,6] := Point(-1,6);

    reserveSlotTV[0,-2,0] := Point(0,0);
    reserveSlotTV[0,-2,1] := Point(0,1);
    reserveSlotTV[0,-2,2] := Point(0,2);
    reserveSlotTV[0,-2,3] := Point(0,4);// attenzione
    reserveSlotTV[0,-2,4] := Point(0,5);
    reserveSlotTV[0,-2,5] := Point(0,6);// una in meno , non sono 7
    //in tutto sono 20  . adesso team 1
    reserveSlotTV[1,-4,0] := Point(13,0);
    reserveSlotTV[1,-4,1] := Point(13,1);
    reserveSlotTV[1,-4,2] := Point(13,2);
    reserveSlotTV[1,-4,3] := Point(13,3);
    reserveSlotTV[1,-4,4] := Point(13,4);
    reserveSlotTV[1,-4,5] := Point(13,5);
    reserveSlotTV[1,-4,6] := Point(13,6);

    reserveSlotTV[1,-3,0] := Point(12,0);
    reserveSlotTV[1,-3,1] := Point(12,1);
    reserveSlotTV[1,-3,2] := Point(12,2);
    reserveSlotTV[1,-3,3] := Point(12,3);
    reserveSlotTV[1,-3,4] := Point(12,4);
    reserveSlotTV[1,-3,5] := Point(12,5);
    reserveSlotTV[1,-3,6] := Point(12,6);

    reserveSlotTV[1,-2,0] := Point(11,0);
    reserveSlotTV[1,-2,1] := Point(11,1);
    reserveSlotTV[1,-2,2] := Point(11,2);
    reserveSlotTV[1,-2,3] := Point(11,4);// attenzione
    reserveSlotTV[1,-2,4] := Point(11,5);
    reserveSlotTV[1,-2,5] := Point(11,6);// una in meno , non sono 7

end;
procedure TSoccerbrain.createAIfield ;
begin
  ...
  if CanDoSub ( Team ) then begin  //not gk
    ...
    if anInjured <> nil then begin
      Result := AI_Think_sub ( team, anInjured, possiblysamerole ); //result = abs4, SubDone, None
        if (Result = SubAbs4) or (Result = SubDone) then
        Exit; // abs4 passo al giro dopo
    end;
...
var
  tmp: TStringList;
  cof,fkf1,fkf2,fkf3,fkf4: string;
  ShpOrLop,PosChance: Integer;
  dstCell: TPoint;
  CornerMap: TCornerMap;
...
var
  MidChance : TAIMidChance;
  MidChances : TList<TAIMidChance>;
  ShpOrLop,ShpOrLopOrPlm,MoveValue: Integer;
...
var
  dstCell: TPoint;
  aVolleyFriend,aHeadingfriend,anOpponent,aFriend: TSoccerPlayer;
  aDoor: TPoint;
  ShotOrPlm,CrossOrlopVolley,MoveValue,ShotOrDri,ShotOrFindCross,PlmOrFindCross: Integer;
  aplmCell: TPoint;
...
var
  anOpponent,aFriend: TSoccerPlayer;
  aDoor: TPoint;
begin
...
begin
  ...
  // qui hho tutti e 2 disponibili
    ...
    if Ball.Player.Passing >= 6 then  // se posso fare 10
...
begin
  ...
  for I := 0 to TVCrossingAreaCells.Count -1 do begin
    ...
    if aCrossCell.DoorTeam <> aPlayer.Team then begin // <> opponent
      ...
      and  ( AbsDistance( Ball.Player.CellX,ball.Player.CellY, aCrossCell.CellX , aCrossCell.cellY ) <= (CrossingRangeMax + ball.Player.tal_longpass ) )
      then begin
          ...
          for P:= 0 to Ball.Player.MovePath.Count - 1 do begin
            ...
            for t := 0 to LstAutoTackle.Count -1 do begin
                ...
                if (LstAutoTackle[t].Cell.X = Ball.Player.MovePath[p].X) and (LstAutoTackle[t].Cell.Y = Ball.Player.MovePath[p].Y) then begin // se la cella lo riguarda. questo fa la giusta animazione
                    ...
                    CalculateChance  (Ball.player.Defense  +Ball.player.Tal_Toughness  , LstAutoTackle[t].Player.Defense
                    + LstAutoTackle[t].Player.tal_challenge + Modifier_autotackle , chanceA,chanceB);
...
begin
    ...
    AICell := Tv2AiField (Ball.player.Team, Ball.player.CellX,Ball.player.CellY ) ;
    MaxDistance := LoftedPassRangeMax + Ball.Player.tal_longpass;
    ...
    for Y := MaxDistance downto MaxDistance-1 do begin // fino a lop max -1 (abbastanza lontano in avanti)
      for X := 0 to 6 do begin // tutta la linea possibile X
        ...
        TvCell := AiField [ Ball.player.Team, dstCell.X,dstCell.Y ];
        if AbsDistance( Ball.player.CellX,Ball.player.CellY,TvCell.X,TvCell.Y) > (LoftedPassRangeMax + Ball.Player.tal_longpass) then Continue;
        if IsOutSide(TvCell.X, TvCell.Y ) then Continue;

          TvCell := AiField [ Ball.player.Team, dstCell.X,dstCell.Y ];
          if GetSoccerPlayer (TvCell.X,TvCell.Y) = nil then begin
            aList.Add(dstCell);
            continue;
          end;
          if (GetSoccerPlayer (TvCell.X,TvCell.Y).Team = Ball.player.Team) and ( GetSoccerPlayer (TvCell.X,TvCell.Y).Ids <>  Ball.Player.ids  ) then begin // se è un compagno
            aList.Add(dstCell);
            continue;
          end;
        end;
    ...
    if aList.Count > 0 then begin
      ...
      TvCell := AiField [ Ball.player.Team, Result.X, Result.Y ];
      Result := TvCell;
    end;
    aList.Free;
    ...
    for Y := 1 to 5 do begin // fino a lop 1 una cella libra la trovo per forza
      for X := 0 to 6 do begin // tutta la linea possibile X
        ...
        if not IsOutSide( TvCell.X, TvCell.Y )  then begin // se è dentro il campo e se è friendly/vuota
          ...
          else if DummyFriend.Ids <>  Ball.Player.ids   then begin // se è un compagno
            aList.Add(dstCell);
//            continue;
          end;
        end;
...
begin
    ...
    AICell := Tv2AiField (Ball.player.Team, Ball.player.CellX,Ball.player.CellY ) ;
    MaxDistance := ShortPassRange + Ball.Player.tal_longpass;
    aList:= Tlist<TPoint>.Create;
    ...
    for Y := MaxDistance downto 1 do begin // fino a lop max -1 (abbastanza lontano in avanti)
      for X := 0 to 6 do begin // tutta la linea possibile X
        ...
        (*absdistance è diversa dal rangeMAx, va checkata ora*)
        TvCell := AiField [ Ball.player.Team, dstCell.X,dstCell.Y ];
        if AbsDistance( Ball.player.CellX,Ball.player.CellY,TvCell.X,TvCell.Y) > (ShortPassRange + Ball.Player.tal_longpass) then Continue;
        if IsOutSide(TvCell.X, TvCell.Y ) then Continue;
          ...
          else if (DummyFriend.Ids <>  Ball.Player.ids) and (Dummyfriend.Role <> 'G')   then begin // se è un compagno
            aList.Add(dstCell);
            continue;
          end;
      end;
...
begin
  ...
  for I := lstSoccerPlayer.Count -1 downto 0 do begin
    ...
    if (aFriend.Team <> team) or (aFriend.Role = 'G') or (aFriend.Ids = meIds) or ( not aFriend.CanMove ) or ( aFriend.Speed < dist)  then continue;
...
begin
    ...
    for I := lstSoccerPlayer.Count -1 downto 0 do begin
      if ( not lstSoccerPlayer[i].PressingDone ) and (lstSoccerPlayer[i].canSkill) and (lstSoccerPlayer[i].Role <> 'G') and
          (AbsDistance(lstSoccerPlayer[i].CellX, lstSoccerPlayer[i].CellY, Ball.Player.CellX,Ball.Player.CellY )=1) and
           (lstSoccerPlayer[i].Team = team) and ( lstSoccerPlayer[i].Tal_Experience > 00 )
        then begin
          Result := lstSoccerPlayer[i];
          Exit;
        end;
...
var
  i,dist,MinDist,idx,aRnd,p: Integer;
  FriendlyWall,FinalWall,OpponentWall: boolean;
  aPlayer: TSoccerPlayer;
...
begin
  ...
  for x := 0 to 6 do begin
    for y := 0 to 11 do begin
      if (Aifield[Team, X,Y ].X = tvX) and (Aifield[Team, X,Y ].Y = tvY)then begin
        Result.X := X;
        Result.Y := Y;
        Exit;
      end;
...
//      aPlayer.grouped := false;
    ...
    if aList.Count > 0 then begin
      ...
      for I := 0 to aList.Count -1 do begin
        ...
        IntTostr(aPlayer.Speed)+',Speed,'+ aList[i].ids+','+IntTostr(Roll.value));
      end;
                                        ...
                                        case GetTeamBall of  // di quale team è la palla
                                          -1: begin  // palla neutra
                                            if aPlayer.Role ='D' then begin       // metacampo 0, teamturn 0, aPlayer.team 0, palla neutra
                                              ...
                                              if aPlayer.Tal_midOffensive > 0 then begin
                                                   AI_MovePlayer_DefaultX_plus_1 ( aPlayer  ) ;
                                              end
                                              else if aPlayer.Tal_midDefensive > 0 then begin
                                            ...
                                            //dif	ball-1
                                              ...
                                              if aPlayer.Tal_midDefensive > 0 then begin
...
//                                            dif	defaultX
                                          ...
                                          //dif	p-1palla
                                              if aPlayer.Tal_midOffensive > 0 then Continue;
                                      ...
                                      0: begin                                   // metacampo 0, teamturn 1,  aPlayer.team 0, palla team team 0
                                        if aPlayer.Role ='D' then begin
                                          ...
                                          //dif	default
                                                ...
                                                else if aPlayer.Tal_midOffensive > 0 then begin
                                                   AI_MovePlayer_DefaultX ( aPlayer  ) ;
                                                end;
                                        end;
...
//      in ai_moveall prima fa aimoveall poi cerca la cella Y precisa defaultcellY ,  . spende di più a correr . utile per chi deve stare sulle fascie
                if ( aPlayer.Tal_Positioning > 0 )  and ( not toball)  then begin  // o not aPlayer.hasball
   ...
   { Playmaker GETFAVOURCELLPATH si avvicina al proprio compagno portatore di palla}
                ...
                else if ( aPlayer.Tal_PlayMaker > 0 )  and ( not toball)  then begin  // o not aPlayer.hasball
                ...
                else if ( aPlayer.Tal_Marking > 0 )  and ( not toball)  then begin  // o not aPlayer.hasball
...
begin
  ...
  for I := lstSoccerPlayer.Count -1 downto 0 do begin
    ...
    if (anOpponent.CellX = aCell.X)  and (anOpponent.CellY = aCell.Y) then begin
      ...
      TsScript.add ('sc_player,'+ anOpponent.Ids +','+IntTostr(aCell.X)+','+ IntTostr(aCell.Y)+','+
      IntTostr(anOpponent.CellX)+','+ IntTostr(anOpponent.CellY)  ) ;
    end;
  end;
end;
...
function TSoccerbrain.NextReserveSlot ( aPlayer: TSoccerPlayer): Tpoint;
var
  x,y: Integer;
begin
    ...
    for x := -4 to -1 do begin
      for y := 0 to 6 do begin
        ...
        if ReserveSlot [aPlayer.team,x,y] = '' then begin
          Result := Point(x,y);
          Exit;
        end;
      end;
...
var
  aPoint: TPoint;
begin
    aPoint:= NextReserveSlot (aPlayer);
    ReserveSlot [aPlayer.Team, aPoint.X,aPoint.Y]:= aPlayer.Ids;
    aPlayer.Cells := aPoint;
    aPlayer.DefaultCells := aPoint;
    aPlayer.AIFormationCellX := aPoint.X;
    aPlayer.AIFormationCellY := aPoint.Y;
    aPlayer.Role := 'N';
end;
