...
{$ifdef tools}
  if Gamemode = pvp then begin
    ...
    if MyBrain.Minute < 120
      then MyBrain.FlagEndGame:= False;
  end;
{$endif tools}
...
//  CreateRewards;
  ...
  if not Directoryexists( ExtractFilePath(application.exename) + 'saves') then
    mkdir (ExtractFilePath(application.exename) + 'saves');
...
procedure Tform1.HHFP_Friendly ( Team: integer );
var
  i,CellX: integer;
  aFieldPointSpr : SE_Sprite;
...
procedure TForm1.ShowFreeKickArrows ( CellX,CellY: integer );
var
  aFieldPoint: SE_Sprite;
begin
  SE_interface.RemoveAllSprites;
...
begin
  ...
  bmp.Bitmap.Canvas.Brush.Color := $20430f;//$3a751d;
  bmp.Bitmap.Canvas.FillRect(Rect(0,0,bmp.Width,bmp.Height));
  ...
  // se è una cella non della propria formazione non faccio nulla.
    ...
    // se non è il GK escludo quella cella.
      if aPlayer.TalentID1 <> TALENT_ID_GOALKEEPER then Begin
        ...
        // solo del mio team , sono in screenformation quindi le celle sono per froza in 0 2 5 8
...
begin
  ...
  case MouseWaitFor of
    ...
    WaitForXY_FKF2: begin  // chi batte il freekick con cross
      ...
      ShowFreeKickArrows (  CornerMap.HeadingCellA [0].X,CornerMap.HeadingCellA [0].Y);
      TsCoa.add (aSpriteClicked.guid) ; // ids
    ...
    WaitForXY_FKA2: begin
      ...
      if tsCoa.Count = 4 then begin   // cof + 3 coa
        ...
        ShowFreeKickArrows (  CornerMap.HeadingCellA [TsCoa.count-1].X,CornerMap.HeadingCellA [TsCoa.count-1].Y);
      end;
    end;
    WaitForXY_FKD2: begin
      ...
      if tsCod.Count = 3 then begin  // 3 cod
        ...
        ShowFreeKickArrows ( CornerMap.HeadingCellD [TsCod.count].X,CornerMap.HeadingCellD [TsCod.count].Y);
      end;
    end;
    ...
    WaitForXY_CornerCOD: begin
      ...
      if tsCod.Count = 3 then begin  // 3 cod
        ...
        ShowFreeKickArrows (    CornerMap.HeadingCellD [TsCod.count-1].X,CornerMap.HeadingCellD [TsCod.count-1].Y);
      end;
    end;
  end;
end;
...
begin
  ...
  // mbleft
  if MouseWaitFor = WaitForXY_TACTIC1 then
    exit
...
procedure TForm1.ScreenTactics_SE_Players ( aSpriteClicked: SE_Sprite; Button: TMouseButton  );
var
  bmp : SE_Bitmap;
begin
...
//    OutputDebugString( PChar(lstSprite[i].Engine.Name ) );
    if lstSprite[i].Engine = SE_YesNo then begin     // YesNo gestiti a parte
      ...
      if lstSprite[i].Engine = SE_Green then begin  //se_Green arriva prima di tutti
        ...
        ScreenTactics_SE_Players ( lstSprite[i], Button );// click che seleziona se_dragGuid . segue mouseup
        Exit;
      end
  ...
  //  ReleaseMutex(MutexAnimation );
   ...
   // SE_FieldPointsReserve.HiddenSpritesMouseMove := true;
    ...
    for I := 0 to MyBrain.lstSoccerPlayer.Count -1  do begin
      ...
      if AbsDistance(aPlayer.CellX, aPlayer .CellY, MyBrain.Ball.CellX ,MyBrain.Ball.celly) < 4 then begin
        aPlayer.se_sprite.Visible := False;
        aFieldPointSpr := SE_FieldPoints.FindSprite( IntToStr(aPlayer.CellX)+'.'+ IntToStr(aPlayer.CellY ));
        aFieldPointSpr.Visible := False;
      end;
    end;
    ...
    for M := 1 to DivisionMatchCount[D]  do begin
      ...
      if (StrToInt(ts2[0]) = MyGuidTeam) or (StrToInt(ts2[2]) = MyGuidTeam) then begin   // se è la mia partita
        ...
        if not simulation then begin
            ...
            pveSynchBrain; // sincronizza la grafica sprite all'ultimo incMove
            WaitForSingleObject(MutexPveSyncBrain,INFINITE);
            ReleaseMutex(MutexClientLoadbrain);
            SpriteReset;
            GameScreen := ScreenLive;
...
begin
 ...
 // lstText := TStringList.Create;
  ...
  AssignFile( fileName,  dir_Help + Language +'\'+ Help +'.txt');
  Reset(fileName);
